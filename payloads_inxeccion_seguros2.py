...
# --- NODO 4: Reporte Ejecutivo con Matriz de Riesgos ---
def security_reporter_node(state: SecurityState):
    print("[PASO 4]: Consolidando reporte final y generando Matriz de Riesgos...")
    
    prompt = f"""
    Eres un Auditor Senior de Seguridad. Basado en los hallazgos anteriores, genera un reporte que incluya:
    
    1. Un resumen ejecutivo del estado de seguridad.
    ...
    2. Una TABLA DE MATRIZ DE RIESGOS con las siguientes columnas:
       | Punto de Entrada | Parámetro | Tipo de Riesgo | Defensa Detectada | Estado |
       
       - En 'Defensa Detectada', menciona herramientas específicas encontradas (Pydantic, JWT, SQLAlchemy, etc.).
       - En 'Estado', usa '✅ SEGURO' si hay defensas o '❌ VULNERABLE' si no las hay.

    HALLAZGOS TÉCNICOS:
    {state['security_payloads']}
    
    AUDITORÍA DE DEFENSAS:
    {state['validation_audit']}
