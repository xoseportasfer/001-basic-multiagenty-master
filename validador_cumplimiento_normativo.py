...
def compliance_miner_node(state: ComplianceState):
    ...
    try:
        ...
        for root, _, files in os.walk(temp_dir):
            for f in files:
                if f.endswith((".py", ".env", ".sql")):
                    with open(os.path.join(root, f), "r", encoding="utf-8", errors="ignore") as file:
                        # Buscamos archivos que definan modelos o configuraciones
                        content = file.read()
                        if any(key in content.lower() for key in ["user", "email", "password", "phone", "address", "card"]):
                            code_content += f"\nFILE: {f}\n{content[:2000]}\n" # Limitamos contexto
    finally:
